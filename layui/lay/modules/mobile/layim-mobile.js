...
layui.define(['laytpl', 'upload', 'layer-mobile', 'zepto'], function(exports){
  ...
  var SHOW = 'layui-show', THIS = 'layim-this', MAX_ITEM = 60;
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ...
      //,'<span  class="layui-icon layim-send" title="辅助键盘" layim-event="phone" >&#xe678;</span>'
     
      ,'<textarea type="text" autocomplete="off" ></textarea>'
      
      ,'<span  class="layui-icon layim-send" title="选择表情" layim-event="face" >&#xe60c;</span>'
      ,'<button class="layim-send layui-disabled" layim-event="send">发送</button>'
      ,'</div>'

      ,'<div class="layim-chat-tool" style="display:none" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}" >'
  ...
  var thisChat = function(){
    ...
    return {
      ...
      ,footer: cont.find('.layim-chat-footer')
    };
  };
  ...
  var sendMessage = function(){
    ...
    textarea.next().next().addClass('layui-disabled');
  };

  // 设置footer 高度
  var  setFooterHight= function(textarea){
    var line =  textarea.split("\n").length;
    if (line<=1){
      line = 1
    }else if (line>3){
      line = 3
    ...
    var footer = thatChat.footer;
    footer.height((24+(line-1)*15+3)+'px')
    textarea.height( (30+(line-1)*15)+'px')
  ...
  var hotkeySend = function(){
    ...
    textarea.off('keyup').on('keyup', function(e){
     ...
     //console.log("行数:"+line);
      ...
      setFooterHight(textarea.val());
    });

   // textarea.scrollTop("90px")
  };
      ...
      layer.popBottom({
        ...
        ,success: function(elem){
          ...
          touch(list, function(){
            ...
            input.next().next()[input.val() === '' ? 'addClass' : 'removeClass']('layui-disabled');
            return false;
          });
        }
      });
  ...
  window.closePanel = events.back
  
  //暴露接口
  exports('layim-mobile', new LAYIM());

}).addcss(
  'modules/layim/mobile/layim.css?v=2.20'
  ,'skinlayim-mobilecss'
);